...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    // Pre-fill email if coming from passwordless registration
  ...
  // Enhanced sendCode function that handles both existing users and redirects new users
  const sendCode = async () => {
    ...
    try {
      ...
      // Handle the 400 response which might be success or user doesn't exist
      if (error.response?.status === 400) {
        ...
        if (data?.ui?.messages) {
          ...
          // Check if user doesn't exist (strategy error)
          const errorMessage = data.ui.messages.find((msg: any) => 
            msg.type === 'error' && (
              msg.text?.includes('strategy') || 
              msg.text?.includes('identifier') ||
              msg.text?.includes('account does not exist')
            )
          )
          
          if (errorMessage) {
            // User doesn't exist, redirect to passwordless registration
            router.push(`/register-passwordless?email=${encodeURIComponent(email)}`)
            return
          }
        }
      }
      ...
      setErrors({ 
        general: 'Unable to send login code. You might need to register first.' 
      })
    } finally {
      setIsSubmitting(false)
    }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-md w-full space-y-8">
        ...
        {/* Passwordless Registration CTA */}
        <div className="text-center py-2">
          <a 
            href="/register-passwordless"
            className="inline-flex items-center text-blue-600 hover:text-blue-500 font-medium text-sm group"
          >
            <Sparkles className="w-4 h-4 mr-1 group-hover:animate-pulse" />
            New here? Join with just your email →
          </a>
        </div>
            ...
            {showCodeLogin && (
              <div className="space-y-4 pt-4 border-t border-gray-200">
                ...
                <button
                  ...
                  className="w-full py-2 px-4 border border-blue-600 text-blue-600 hover:bg-blue-50 rounded-md text-sm font-medium disabled:opacity-50 transition-colors"
                >
                  {codeSent ? 'Code sent! Check your email' : 'Get a login code instead'}
                </button>
                  ...
                  <form onSubmit={handleCodeSubmit} className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        } placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-blue-500 focus:border-blue-500 text-center`}
